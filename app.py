...
from evisionary_common import MISSING_LIKE, KNOWN_SCIENTIFIC
...
DISPLAY_COLUMNS = [
    ...
    "molecule_raw_norm_discrepant", "molecule_norm_alias_discrepant",
]
...
def species_to_html(val):
    """
    Return an XSS-safe HTML string with recognised binomials italicised.
    Unrecognised tokens are HTML-escaped to prevent injection.
    Multi-species values separated by '|' are handled per token.
    """
    text = clean_text(val)
    if text == "Unknown":
        return "Unknown"
...
def row_to_output(rd):
    species_plain = clean_text(rd.get("species"))
    return {
        ...
        "species": species_plain,                       # plain text (safe fallback)
        "species_html": species_to_html(rd.get("species")),  # italicised, XSS-safe
...
def search_duckdb(query=None, limit=DEFAULT_LIMIT, source_filter=None,
    ...
    flags = [bool(query), bool(source_filter), bool(species_filter), bool(molecule_group_filter)]
    if not any(flags):
        return []

    sql = build_search_sql(*flags)
    params = build_query_params(query, source_filter, species_filter, molecule_group_filter)
    params.append(limit)
